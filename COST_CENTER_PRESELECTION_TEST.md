...
console.log('Purchase Cost Center:', '<?= $purchase->cost_center_id ?? "None" ?>');
...
console.log('Available Options:', $('#cost_center_id option').map(function() { return this.value + ': ' + this.text; }).get());
```
