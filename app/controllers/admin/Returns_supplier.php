...
class Returns_supplier extends MY_Controller
{
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->returns_supplier_model->updateReturn($id, $data, $products)) {
            ...
            $this->session->set_userdata('remove_rlls', 1);
            $this->session->set_flashdata('message', lang('return_added'));
            admin_redirect('returns_supplier?lastInsertedId='.$id);
        } else {
            $this->data['inv'] = $inv;
            ...
            foreach ($inv_items as $item) {
                ...
                $row->cost = $this->sma->formatDecimal($item->cost_price);
                $row->sale_price = $this->sma->formatDecimal($item->net_unit_price);
    ...
    public function getReturns($warehouse_id = null)
    {
        ...
        if($this->Owner || $this->GP['supplier-returns-edit']){
            $action .= '<li>' . $edit_link . '</li>';
        }
        if($this->Owner || $this->GP['supplier-returns-delete']){
    ...
    public function bch_suggestions()
    {
        ...
        // (01)03664798044249(21)A07GPEY6PHNFDE(10)240413(17)270228
