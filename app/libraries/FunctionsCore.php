...
Class FunctionsCore {
	...
	**/
	public function ledgerList($entrytypeLabel, $searchTerm = null, $selectedLedgers = array()) {
		...
		$entrytype = $this->_ci->db->query("SELECT * FROM ".$this->_ci->db->dbprefix('accounts_entrytypes')." WHERE label='$entrytypeLabel'");
	...
	**/
	...
	{
		$this->_ci->db->where('sma_accounts_entrytypes.id', $entrytype_id);
		$row = $this->_ci->db->get('sma_accounts_entrytypes')->row();
	...
	*/
	...
	function showTag($id) {
		...
		$tag = $this->_ci->db->get('sma_accounts_tags')->row_array();
	...
	function entryLedgers($id) {
		$this->_ci->load->admin_model('Entry_model');
