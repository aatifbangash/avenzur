	...
	public function __construct()
  	{
      	...
      	$this->_ci->load->admin_model('ledger_model');
      	$this->_ci->load->language('admin/accounts');
 ...
 */
	function build($id)
	{
		if ($id == 0)
		{
			...
			$group = $this->_ci->db->where('sma_accounts_groups.id', $id)->get('sma_accounts_groups')->row_array();
			$this->id = $group['id'];
 ...
 */
	function add_sub_groups() {
		if ($this->for == 'select2' && $this->searchTerm) {
			$this->_ci->db->like('sma_accounts_groups.code', $this->searchTerm);
			$this->_ci->db->or_like('sma_accounts_groups.name', $this->searchTerm);
		} else {
			$this->_ci->db->where('sma_accounts_groups.parent_id', $this->id);

			/* If primary group sort by id else sort by name */
			if ($this->id == NULL) {
				// $this->_ci->db->order_by('groups.id', 'asc');
				// $this->_ci->db->order_by('groups.code', 'asc');
				// $child_group_q = $this->_ci->db->get('groups')->result_array();
			} else {
				// $this->_ci->db->order_by('groups.name', 'asc');
				// $this->_ci->db->order_by('groups.code', 'asc');
				// $child_group_q = $this->_ci->db->get('groups')->result_array();
			}
		}
 ...
 */
	function add_sub_ledgers() {
		...
		if ($this->for == 'select2' && $this->searchTerm) {
			$this->_ci->db->like('sma_accounts_groups.code', $this->searchTerm);
			$this->_ci->db->or_like('sma_accounts_groups.name', $this->searchTerm);
			$child_group_l = $this->_ci->db->get('sma_accounts_groups')->num_rows();			
		} else {
			$child_group_l = 1;
		}
		...
		if ($child_group_l == 0) {
			...
			$this->_ci->db->like('sma_accounts_ledgers.code', $this->searchTerm);
			$this->_ci->db->or_like('sma_accounts_ledgers.name', $this->searchTerm);
		} else {
			$this->_ci->db->where('sma_accounts_ledgers.group_id', $this->id);
		...
		$this->_ci->db->order_by('sma_accounts_ledgers.code', 'asc');
		// $this->_ci->db->order_by('ledgers.name', 'asc');
		$child_ledger_q = $this->_ci->db->get('sma_accounts_ledgers')->result_array();
