...
class Site extends CI_Model
{
    ...
    public function checkPermissions()
    {
        ...
        return $this->getLegacyPermissionsRow((int) $this->session->userdata('group_id'));
    }
    ...
    private function getPermissionAssignments($filters = [])
    {
        ...
        if (array_key_exists('group_id', $filters)) {
            $this->db->where('permission_assignments.group_id', (int) $filters['group_id']);
            $this->db->where('permission_assignments.user_id IS NULL', null, false);
        }

        if (array_key_exists('user_id', $filters)) {
            $this->db->where('permission_assignments.user_id', (int) $filters['user_id']);
        }
    ...
    private function getLegacyPermissionsRow($groupId)
    {
        $q = $this->db->get_where('permissions', ['group_id' => $groupId], 1);
        if ($q->num_rows() > 0) {
    ...
    public function getParentCompanyByGroupAndId($group_name, $id)
{
    ...
    public function reportCustomerLedgerWarehouseCondition($warehouse_id, $entries_alias = 'sma_accounts_entries')
    {
        ...
        $memoPaymentJournalSubquery = function () use ($pr, $pay) {
            return "SELECT DISTINCT pr.journal_id
                    ...
                    INNER JOIN {$pay} pay
                        ON pay.payment_id = pr.id
                    WHERE pr.journal_id IS NOT NULL
                    AND NULLIF(pay.memo_id, '') IS NOT NULL
                    AND NULLIF(pay.memo_id, 0) IS NOT NULL";
        };

        // Debit/Credit memos are not warehouse specific
        ...
        if ($warehouse_id) {
            ...
            return "(
                (NULLIF({$sid}, '') IS NOT NULL
                    AND NULLIF({$sid}, 0) IS NOT NULL
                    AND {$sid} IN (
                        SELECT id
                        FROM {$sales}
                ...
                (NULLIF({$rid}, '') IS NOT NULL
                    AND NULLIF({$rid}, 0) IS NOT NULL
                    AND {$rid} IN (
                        SELECT id
                        FROM {$returns}
                        WHERE warehouse_id = {$wh}
        ...
        return "(
            (
                NULLIF({$sid}, '') IS NULL
                OR NULLIF({$sid}, 0) IS NULL
                OR {$sid} NOT IN (
                    SELECT id
                    FROM {$sales}
                    WHERE warehouse_id = {$osw_id}
                )
            )
