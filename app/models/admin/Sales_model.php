        ...
        if ($this->db->insert('sales', $data)) {
            ...
            foreach ($items as $item) {
                ...
                if ($data['sale_status'] == 'completed'){ //handle inventory movement 
                    ...
                    $this->Inventory_model->add_movement($item['product_id'], $item['batch_no'], 'sale', $item['quantity'], $item['warehouse_id'], $sale_id, $item['net_cost'], $item['expiry'], $item['net_unit_price'], $real_cost, $item['avz_item_code'], NULL, $data['customer_id']);
                } 
        ...
        if ($this->db->update('sales', $data, ['id' => $id]) && $this->db->delete('sale_items', ['sale_id' => $id]) && $this->db->delete('costing', ['sale_id' => $id])) {
            ...
            foreach ($items as $item) {
                ...
                if ($data['sale_status'] == 'completed' && $this->site->getProductByID($item['product_id'])) {
                    ...
                    $this->Inventory_model->add_movement($item['product_id'], $item['batch_no'], 'sale', $item['quantity'], $item['warehouse_id'], $id,  $item['net_cost'], $item['expiry'], $item['net_unit_price'], $real_cost, $item['avz_item_code'], NULL, $data['customer_id']); 
                    $item_costs = $this->site->item_costing($item);
                    foreach ($item_costs as $item_cost) {
                        if (isset($item_cost['date']) || isset($item_cost['pi_overselling'])) {
                            $item_cost['sale_item_id'] = $sale_item_id;
