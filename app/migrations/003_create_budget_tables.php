<?php
/**
 * Migration: Create Budget Module Tables
 * 
 ...
 * 
 * Date: 2025-10-25
 * Timeline: Phase 1 & 2 combined (1 day sprint)
 */

class Migration_003_create_budget_tables extends CI_Migration {
    ...
    public function up() {
        ...
        $this->db->query("
            CREATE TABLE IF NOT EXISTS `sma_budget_alert_config` (
                ...
                `notification_channels` JSON DEFAULT '\"email\"' COMMENT 'email|sms|in-app|webhook',
        ...
        $this->db->query("
            ...
            SELECT 
                ...
                ba.allocated_amount,
                bt.actual_spent,
                ba.allocated_amount - bt.actual_spent AS remaining_amount,
                CASE 
                    WHEN ba.allocated_amount = 0 THEN 0
                    ELSE ROUND((bt.actual_spent / ba.allocated_amount) * 100, 2)
        ...
        // ============================================================
        // VIEW: Budget Summary by Entity (Company/Pharmacy/Branch)
        // ============================================================
        $this->db->query("
            ...
            SELECT 
                ...
                SUM(bt.actual_spent) as total_spent,
                SUM(ba.allocated_amount) - SUM(bt.actual_spent) as total_remaining,
                ROUND(SUM(bt.actual_spent) / SUM(ba.allocated_amount) * 100, 2) as overall_percentage,
                COUNT(CASE WHEN bt.status = 'safe' THEN 1 END) as safe_count,
        ...
        $this->db->query("
            ...
            SELECT 
                ...
                bae.acknowledged_at,
                bt.status as current_status,
                bt.percentage_used as current_percentage,
                bf.risk_level
    ...
    public function down() {
        // Drop views first (reverse order of dependencies)
        $this->db->query("DROP VIEW IF EXISTS view_budget_alerts_dashboard");
        $this->db->query("DROP VIEW IF EXISTS view_budget_summary");
        $this->db->query("DROP VIEW IF EXISTS view_budget_vs_actual");
        
        // Drop tables
        $this->db->query("DROP TABLE IF EXISTS sma_budget_audit_trail");
        $this->db->query("DROP TABLE IF EXISTS sma_budget_alert_events");
        $this->db->query("DROP TABLE IF EXISTS sma_budget_alert_config");
        $this->db->query("DROP TABLE IF EXISTS sma_budget_forecast");
        $this->db->query("DROP TABLE IF EXISTS sma_budget_tracking");
        $this->db->query("DROP TABLE IF EXISTS sma_budget_allocation");
