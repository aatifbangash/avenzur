...
FROM sma_sales ss
WHERE ss.warehouse_id IS NOT NULL
  AND ss.sale_status IN ('completed', 'completed_partial')
GROUP BY 
    ss.warehouse_id,
    YEAR(ss.date),
    MONTH(ss.date);
...
WHERE sp.warehouse_id IS NOT NULL
  AND sp.status IN ('received', 'received_partial', 'pending')
GROUP BY 
    sp.warehouse_id,
    YEAR(sp.date),
    MONTH(sp.date);
...
SELECT 
    ...
    vsm.period,
    SUM(COALESCE(vsm.total_sales_amount, 0)) AS kpi_total_revenue,
    SUM(COALESCE(vpm.total_purchase_cost, 0)) AS kpi_total_cost,
    SUM(COALESCE(vsm.total_sales_amount, 0)) - SUM(COALESCE(vpm.total_purchase_cost, 0)) AS kpi_profit_loss,
    CASE 
        WHEN SUM(COALESCE(vsm.total_sales_amount, 0)) = 0 THEN 0
        ELSE ROUND(((SUM(COALESCE(vsm.total_sales_amount, 0)) - SUM(COALESCE(vpm.total_purchase_cost, 0))) / SUM(COALESCE(vsm.total_sales_amount, 0))) * 100, 2)
    END AS kpi_profit_margin_pct,
