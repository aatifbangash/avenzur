        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $.ajax({
                    type: "POST",
                    data: entryitem_data,
                    url: '<?=admin_url("entries/addentry/"); ?>',
                    success: function(data) {
                        if (data) {
                            var tbody = $("#entryitems");
          ...
          <div class="box">
            ...
            <div class="box-body">
                ...
                <?php
                    ...
                    ?>
                        <select name="tag_id" class="form-control" tabindex="3">
                            ...
                            if (isset($curEntryitems) && !empty($curEntryitems) && sizeof($curEntryitems, 1) > 5 ) {
                                foreach ($curEntryitems as $row => $entryitem) {
                                    ...
                                    if (empty($entryitem['cr_amount'])) {
                                        echo '<td>' . $entryitem['dr_amount'] . '</td>';
                                        $data = array(
                                            'type' => 'hidden',
                                        ...
                                        echo '<td><strong>-</strong></td>';
                                        echo '<td>' . $entryitem['cr_amount'] . '</td>';
                                        $data = array(
                                            'type' => 'hidden',
                                    ...
                                    echo '<td class="ledger-balance"><div>'. $entryitem['ledger_balance'] .'</div></td>';
                                    $data = array(
                                        'type' => 'hidden',
                    ...
                    echo "<textarea name='notes' class='form-control' tabindex='9' rows='3'></textarea>";
                    echo "</div>";
                    echo '<div class="form-group">';
                    echo form_submit('submit', lang('entries_views_add_label_submit_btn'), array('class'=>'btn btn-success pull-rignt'));
