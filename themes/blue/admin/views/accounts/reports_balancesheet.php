...
function account_st_short($account, $c = 0, $THIS, $dc_type) {
    ...
    if (count($account->children_ledgers) > 0)
    {
        ...
        foreach ($account->children_ledgers as $id => $data)
        {
            ...
            echo anchor('admin/areports/ledgerstatement/ledgerid/'.$data['id'], $CI->functionscore->toCodeWithName($data['code'], $data['name']));
            echo '</td>';
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        startDate = new Date(<?php echo strtotime($this->mAccountSettings->fy_start) * 1000.05532; ?>  + (new Date().getTimezoneOffset() * 60 * 1000));
        endDate = new Date(<?php echo strtotime($this->mAccountSettings->fy_end) * 1000.05532; ?>  + (new Date().getTimezoneOffset() * 60 * 1000));
            ...
            <div class="box-body">
                <div id="accordion">
                    ...
                    <?php echo form_open(); ?>
                        ...
                        <div class="form-group">
                            <input type="reset" name="reset" id="reset" class="btn btn-primary  pull-right" value="<?= lang('clear'); ?>" style="margin-left: 5px;">
                            <input type="submit" name="submit" class="btn btn-primary pull-right" value="<?= lang('create_account_submit_button'); ?>">
                        </div>
                        <?php form_close();  ?>
                    </div>
                </div>
                ...
                <div class="btn-group" role="group">
                   <!-- <a href="<?= base_url('reports/balancesheet/download/pdf/'.$startdate.'/'.$enddate);?>" class="btn btn-default btn-sm"><?= lang('export_to_pdf'); ?></a>
                    <a href="<?= base_url('reports/balancesheet/download/csv/'.$startdate.'/'.$enddate);?>" class="btn btn-default btn-sm"><?= lang('export_to_csv'); ?></a>
                     <a href="<?= base_url('reports/balancesheet/download/xls');?>" class="btn btn-default btn-sm"><?= lang('export_to_xls'); ?></a> -->
                    <a class="btn btn-default btn-sm" onclick="window.print()"><?= lang('print'); ?></a>
                </div>
