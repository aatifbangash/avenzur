...
<div class="incentive-container">
    <div class="incentive-header">
        <h2>Add Incentive</h2>
        <p>Search and add products with incentive percentages for the pharmacist</p>
    </div>
    ...
    <div style="margin-top: 24px;">
        <button type="button" class="btn-save" onclick="saveIncentives(event)">Save Incentives</button>
...
jQuery(document).ready(function($) {
    ...
    window.saveIncentives = function(event) {
        ...
        console.log('Saving incentives:', data);
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo site_url("admin/pharmacist/save_incentive"); ?>',
            dataType: 'json',
            data: data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    alert('Error: ' + (response.message || 'Failed to save incentives'));
                    saveBtn.disabled = false;
                    saveBtn.textContent = 'Save Incentives';
                }
            },
            error: function(xhr, status, error) {
                console.error('Save error:', error);
                alert('Error saving incentives: ' + error);
                saveBtn.disabled = false;
                saveBtn.textContent = 'Save Incentives';
            }
        });
