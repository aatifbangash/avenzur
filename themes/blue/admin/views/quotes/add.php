...
<script type="text/javascript">
    var count = 1, an = 1, product_variant = 0, DT = <?= $Settings->default_tax_rate ?>,
        product_tax = 0, invoice_tax = 0, product_discount = 0, qtitems = {}, order_discount = 0, total_discount = 0, total = 0, allow_discount = <?= ($Owner || $Admin || $this->session->userdata('allow_discount')) ? 1 : 0; ?>,
    ...
    ?>
        ...
        localStorage.setItem('qtcustomer', '<?= $quote->customer_id ?>');
        ...
        localStorage.setItem('qtitems', JSON.stringify(<?= $quote_items; ?>));
        <?php
} ?>
        <?php if ($this->input->get('customer')) {
        ?>
        if (!localStorage.getItem('qtitems')) {
            localStorage.setItem('qtcustomer', <?=$this->input->get('customer'); ?>);
        }
        <?php
    } ?>
        <?php if ($Owner || $Admin) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if(data){
                                        ...
                                        Object.keys(qtitems).forEach(function (key) {
                                            if (qtitems[key].row && qtitems[key].row.avz_item_code === avzItemCode) {
                                                found = true;
                                            }
                                        });
            ...
            if (cn != '') {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        } else if (data.customer_id !== null && data.customer_id !== $('#qtcustomer').val()) {
                            $('#gift_card_no').parent('.form-group').addClass('has-error');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data && data.length > 1) {
                    ...
                    var count = 0;
                    var toitemsStorageValue = JSON.parse(localStorage.getItem('qtitems'));
                    ...
                    itemObj.row.product_id = itemObj.row.product_id || 0;
                    itemObj.row.net_unit_cost = itemObj.row.net_unit_cost || 0;
                    itemObj.row.real_unit_cost = itemObj.row.real_unit_cost || 0;
                    itemObj.row.net_unit_sale = itemObj.row.net_unit_sale || 0;
                    itemObj.row.batch_no = itemObj.row.batch_no || '';
                    //itemObj.row.expiry = itemObj.row.expiry || '';
                    itemObj.row.cost = itemObj.row.cost || 0;
                    ...
                    ?>
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <?php
                                                $wh[''] = '';
                                                foreach ($main_warehouse as $warehouse) {
                    ...
                    echo form_input($warehouse_input);
                        ...
                        <div class="col-md-4">
                           ...
                           <?php if ($Owner || $Admin || $GP['sales-coordinator']) { ?>
                             ...
                             <div class="col-sm-4">
                                <div class="form-group">
                                <?= lang('quote_status', 'quote_status'); ?>
                                <?php $sst = ['open' => lang('open')];
