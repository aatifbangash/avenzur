  ...
  <div class="box-content">
    <div class="row">
      <div class="col-lg-12">
          ...
          <div class="d-flex justify-content-between align-items-center flex-wrap">
            ...
            <div class="d-flex align-items-center mb-2 mb-md-0">
              ...
              <div class="d-flex flex-wrap justify-content-end" style="float: right; margin-left: auto;">
                ...
                <?php if (empty($inv->shopify_synced)): ?>
                <button type="button" class="btn btn-outline-info btn-sm mb-2" onclick="syncPO(<?= $inv->id ?>);">
                  <i class="fa fa-cloud-upload me-1"></i> Sync to Shopify
                </button>
                <?php else: ?>
                <span class="badge bg-success mb-2"><i class="fa fa-check me-1"></i>Already synced</span>
                <?php endif; ?>
        ...
        <!-- SHOPIFY SYNC PREVIEW SECTION -->
        <div class="box mt-4">
          <div class="box-header">
            <h3 class="box-title">
              <i class="fa fa-shopping-cart"></i> Shopify Sync Preview
            </h3>
          </div>
          <div class="box-content">
            <p class="text-muted">
              <small>Below is a preview of the data that will be synced to Shopify when you click the sync button:</small>
            </p>
            
            <!-- Preview Button -->
            <button type="button" class="btn btn-sm btn-info mb-3" onclick="loadShopifySyncPreview(<?= $inv->id ?>);">
              <i class="fa fa-eye me-1"></i> Preview Shopify Data
            </button>

            <div id="syncPreviewContainer" style="display:none;">
              <div class="alert alert-info">
                <strong>Ready to sync?</strong> The following data will be sent to Shopify. Click "Sync to Shopify" button above to proceed.
              </div>
              <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover table-sm">
                  ...
                  <tbody id="shopifySyncPreviewBody">
                    <tr>
                      <td colspan="8" class="text-center text-muted">
                        <em>Click preview button to load data...</em>
                      </td>
                    </tr>
                  </tbody>
                </table>
...
<script>
  ...
  // helper function for syncing a PO to Shopify
  function syncPO(id) {
      if (!id) return;
      
      if (!confirm('Are you sure you want to sync this PO to Shopify?')) {
          return false;
      }
      ...
      $.ajax({
          ...
          success: function(res) {
              if (res.status === 'success') {
                  var msg = res.message || 'PO synced successfully!';
                  if (res.data && Array.isArray(res.data)) {
                      msg += '\nItems synced: ' + res.data.length;
                      console.log('Synced items:', res.data);
                  }
                  alert(msg);
                  location.reload();
                  ...
                  alert(res.message || 'Sync failed - unknown status');
              }
          },
          error: function(xhr, status, err) {
              var error_msg = 'Failed to sync PO to Shopify';
  ...
  // load and display shopify sync preview
  function loadShopifySyncPreview(purchaseId) {
      $.ajax({
          url: '<?= admin_url('purchase_order_sync/sync') ?>',
          type: 'POST',
          data: {
          ...
          success: function(res) {
              ...
              if (res.status === 'error') {
                  alert('Preview Error: ' + res.message);
                  return;
              }
              ...
              if (res.data && Array.isArray(res.data)) {
                  if (res.data.length === 0) {
                      alert('No items to sync for this PO.');
                      return;
                  }
                  
                  var html = '';
                  $.each(res.data, function(idx, item) {
                      var status_badge = item.status === 'update' ? '<span class="badge badge-warning">Update</span>' : '<span class="badge badge-success">Create</span>';
                      html += '<tr>';
                      html += '<td><strong>' + (item.barcode || 'N/A') + '</strong></td>';
                      html += '<td>' + item.name + '</td>';
                      html += '<td style="text-align:right;">' + parseFloat(item.cost_price).toFixed(2) + '</td>';
                      html += '<td style="text-align:right;">' + parseFloat(item.price).toFixed(2) + '</td>';
                      html += '<td style="text-align:right;">' + parseInt(item.initial_stock) + '</td>';
                      html += '<td style="text-align:center;">' + (item.tax_rate || 0) + '%</td>';
                      html += '<td style="text-align:center;">' + status_badge + '</td>';
                      html += '<td style="text-align:center;">' + (item.brand || 'N/A') + '</td>';
                      html += '</tr>';
                  });
                  $('#shopifySyncPreviewBody').html(html);
                  $('#syncPreviewContainer').show();
                  console.log('Loaded ' + res.data.length + ' items for preview');
              } else {
                  alert('No data returned from server.');
              }
          },
          error: function(xhr, status, err) {
              alert('Failed to load preview: ' + (xhr.responseText || err));
              console.error(xhr);
          }
      });
  }
