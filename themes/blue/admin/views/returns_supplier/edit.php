...
<script type="text/javascript">
    ...
    ?>
        ...
        localStorage.setItem('rseref', '<?= $inv->reference_no ?>');
        localStorage.setItem('rsewarehouse', '<?= $inv->warehouse_id ?>');
        localStorage.setItem('rsenote', '<?= str_replace(["\r", "\n"], '', $this->sma->decode_html($inv->note)); ?>');
        localStorage.setItem('rseinnote', '<?= str_replace(["\r", "\n"], '', $this->sma->decode_html($inv->staff_note)); ?>');
        localStorage.setItem('rsediscount', '<?= $inv->order_discount_id ?>');
        localStorage.setItem('rseshipping', '<?= $this->sma->formatDecimal($inv->shipping); ?>');
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i><?= lang('edit_return_supplier'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart('returns_supplier/edit/' . $id, $attrib);
                ?>
                <div class="row">
                    <div class="col-lg-12">
                    ...
                    echo form_input($biller_input);
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php
                                    echo form_input('supplier', (isset($_POST['supplier']) ? $_POST['supplier'] : ''), 'id="rsesupplier" data-placeholder="' . lang('select') . ' ' . lang('supplier') . '" required="required" class="form-control rsupplier input-tip" style="width:100%;"');
                                ?>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang('status', 'status'); ?>
                                <?php
                                if($Owner || $GP['supplier-returns-approve']){ 
