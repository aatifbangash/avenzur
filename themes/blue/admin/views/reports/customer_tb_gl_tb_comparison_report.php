...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-exchange"></i>Customer TB vs GL TB Comparison</h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart('reports/customer_tb_gl_tb_comparison_report', $attrib);
                ?>
                <div class="row">
                    <div class="col-md-4">
        ...
        <?php if (!empty($comparison_result)): ?>
            ...
            $customer_differences = $comparison_result['customer_differences'];
            ...
            <div class="alert alert-info">
                ...
                <strong>GL lines included in Customer TB logic Debit/Credit:</strong>
                <?= number_format((float) $summary['gl_in_tb_total_debit'], 2, '.', ','); ?> /
                <?= number_format((float) $summary['gl_in_tb_total_credit'], 2, '.', ','); ?><br>
                <strong>Customer TB movement Debit/Credit:</strong>
                <?= number_format((float) $summary['customer_tb_total_debit'], 2, '.', ','); ?> /
                <?= number_format((float) $summary['customer_tb_total_credit'], 2, '.', ','); ?><br>
                <strong>Flagged GL rows for manual check:</strong> <?= (int) $summary['manual_flag_count']; ?><br>
                <strong>Customers with amount mismatch:</strong> <?= (int) $summary['customer_mismatch_count']; ?>
            </div>

            <h3>1) GL Lines Missing from Customer TB Logic (Manual Check)</h3>
                        ...
                        <?php if (!empty($flagged_rows)): ?>
                            <?php foreach ($flagged_rows as $row): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?= htmlspecialchars(($row['customer_code'] ?? '') . ' ' . ($row['customer_name'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>
                                        <?= !empty($row['customer_id']) ? ' (#' . (int) $row['customer_id'] . ')' : ''; ?>
                                    </td>
                                    <td>
            ...
            <h3>2) Customer Movement Comparison (GL Included vs Customer TB)</h3>
            <div class="table-responsive">
                <table class="table table-striped table-bordered table-condensed table-hover">
                    <thead>
                        <tr>
                            <th>Customer</th>
                            <th>GL Trs Debit</th>
                            <th>Customer TB Trs Debit</th>
                            <th>Diff Debit</th>
                            <th>GL Trs Credit</th>
                            <th>Customer TB Trs Credit</th>
                            <th>Diff Credit</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (!empty($customer_differences)): ?>
                            <?php foreach ($customer_differences as $row): ?>
                                <tr>
                                    <td>
                                    ...
                                    <td class="text-right"><?= number_format((float) $row['customer_tb_trs_debit'], 2, '.', ','); ?></td>
                                    ...
                                    <td class="text-right"><?= number_format((float) $row['customer_tb_trs_credit'], 2, '.', ','); ?></td>
                                    <td class="text-right"><?= number_format((float) $row['diff_credit'], 2, '.', ','); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="7" class="text-center">No customer-level amount differences found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
        ...
        <?php else: ?>
            <div class="alert alert-warning">
                Select date range and load report to compare customer trial balance against GL lines on all customer receivable ledgers.
            </div>
        <?php endif; ?>
    </div>
</div>
