...
<style>
#sprFilterForm .form-group { margin-bottom: 0; }
.spr-table th  { background: #3c8dbc; color: #fff; white-space: nowrap; }
.spr-table tfoot td { font-weight: 700; background: #f5f5f5; }
.spr-pagination-info { font-size: 13px; color: #555; }
</style>

<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-file-text-o"></i> Supplier Payments Report</h2>
    </div>
    <div class="box-content">

        <!-- ── Filter Form ───────────────────────────────────────── -->
        <form id="sprFilterForm" method="get" action="<?= admin_url('reports/supplier_payments_report') ?>">
            <div class="row" style="margin-bottom:15px; padding:10px 15px; background:#f9f9f9; border:1px solid #e0e0e0; border-radius:4px;">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="supplier_id" style="font-size:12px; font-weight:600;">Supplier</label>
                        <select name="supplier_id" id="supplier_id" class="form-control input-sm select" style="width:100%;">
                            ...
                            <?php if (!empty($suppliers)): foreach ($suppliers as $s): ?>
                                <option value="<?= $s->id ?>" <?= ($filter_supplier_id == $s->id) ? 'selected' : '' ?>>
                                    <?= htmlspecialchars((!empty($s->sequence_code) ? $s->sequence_code . ' - ' : '') . $s->name) ?>
                                </option>
                ...
                <div class="col-md-2 text-right" style="padding-top:22px;">
                    <?php
                    $excel_params = array_filter(['supplier_id' => $filter_supplier_id, 'from_date' => $filter_from, 'to_date' => $filter_to]);
                    $excel_params['export_excel'] = 1;
                    $excel_url = admin_url('reports/supplier_payments_report?' . http_build_query($excel_params));
                    ?>
        ...
        <div class="row" style="margin-bottom:8px;">
            <div class="col-md-6 spr-pagination-info">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover table-striped spr-table" style="margin-bottom:5px;">
                <thead>
                    <tr>
