...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-hand-o-down"></i><?= lang('supplier_advances_report'); ?></h2>
        <?php if (empty($viewtype) || $viewtype != 'pdf') { ?>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <a href="javascript:void(0);" onclick="exportTableToExcel('poTable', 'Supplier_Advances_Balance.xlsx')" class="tip" title="<?= lang('download_xls') ?>">
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart('reports/supplier_advances', $attrib);
                ?>
                        ...
                        <div class="form-group">
                            ...
                            <select name="supplier_ids[]" id="supplier_ids" class="form-control select2" multiple="multiple" data-placeholder="<?= lang('Select Suppliers'); ?>" style="width:100%;">
                                <?php foreach ($suppliers as $supplier): ?>
                                    <option value="<?= $supplier->id; ?>" <?= (isset($selected_suppliers) && in_array($supplier->id, $selected_suppliers)) ? 'selected' : ''; ?>>
                                        <?= ($supplier->company ?? $supplier->name) . ' - ' . $supplier->sequence_code; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                ...
                <div class="alert alert-warning"><?= lang('supplier_advance_ledger_not_configured'); ?></div>
                <?php endif; ?>
                    ...
                    <div class="controls table-controls" style="font-size: 12px !important;">
                        <table id="poTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
                            ...
                            foreach (($trial_balance ?? []) as $data) {
                                ...
                                $detail_url = admin_url('reports/supplier_advance_statement')
                                    . '?supplier=' . (int) $data['party_id']
