$(document).ready(function (e) {
	...
	$("#slpayment_status").change(function (e) {
		...
		if (ps == "partial" || ps == "paid") {
			if (ps == "paid") {
				$("#amount_1").val(
					formatDecimal(
						parseFloat(total + invoice_tax - order_discount + shipping)
					)
				);
			}
			$("#payments").slideDown();
	...
	$(document).on("change", ".rserial", function () {
		...
		qtitems[item_id].row.serial = $(this).val();
		localStorage.setItem("qtitems", JSON.stringify(qtitems));
	});
	...
	$("#slwarehouse").change(function (e) {
		...
		localStorage.removeItem("qtcustomer");
		if ($("#qtcustomer").data("select2")) {
			$("#qtcustomer").select2("destroy");
		}
		$("#qtcustomer").val("");
		nsCustomer();
	});
	...
	$(document).on("change", ".rexpiry", function () {
		...
		if (inputDateObj.getTime() <= yesterdayDate.getTime()) {
			...
			var item_id = $(this).closest("tr").attr("data-item-id");
			qtitems[item_id].row.expiry = $(this).val();
			localStorage.setItem("qtitems", JSON.stringify(qtitems));
		}
	});
		...
		if (site.settings.tax1) {
			...
			if (pr_tax !== null && pr_tax != 0) {
				$.each(tax_rates, function () {
					if (this.id == pr_tax) {
						if (this.type == 1) {
							if (qtitems[item_id].row.tax_method == 0) {
	...
	$(document).on("change", "#punit", function () {
		...
		if (item.units && unit != qtitems[item_id].row.base_unit) {
			$.each(item.units, function () {
				if (this.id == unit) {
	...
	$(document).on("click", "#editItem", function () {
		...
		if (unit != qtitems[item_id].row.base_unit) {
			$.each(qtitems[item_id].units, function () {
				if (this.id == unit) {
					base_quantity = unitToBaseQty($("#pquantity").val(), this);
				}
			});
		...
		qtitems[item_id].row.fup = 1;
		qtitems[item_id].row.qty = quantity;
		qtitems[item_id].row.base_quantity = parseFloat(base_quantity);
		qtitems[item_id].row.real_unit_price = price;
		qtitems[item_id].row.unit = unit;
		qtitems[item_id].row.tax_rate = new_pr_tax;
		qtitems[item_id].tax_rate = new_pr_tax_rate;
		qtitems[item_id].row.discount = discount;
		qtitems[item_id].row.option = $("#poption").val()
			? $("#poption").val()
			: "";
		qtitems[item_id].row.serial = $("#pserial").val();
		localStorage.setItem("qtitems", JSON.stringify(qtitems));
		$("#prModal").modal("hide");
	...
	$(document).on("click", "#sellGiftCard", function (e) {
		if (count == 1) {
			qtitems = {};
			if ($("#slwarehouse").val() && $("#qtcustomer").val()) {
				$("#qtcustomer").select2("readonly", true);
	...
	$(document)
		...
		.on("change", ".rserialno", function () {
			...
			var new_serialno = $(this).val(),
				item_id = row.attr("data-item-id");
			qtitems[item_id].row.serial_number = new_serialno;
			localStorage.setItem("qtitems", JSON.stringify(qtitems));
			loadItems();
		});
	...
	$(document)
		...
		.on("change", ".rbatchno", function () {
			...
			qtitems[item_id].row.expiry = batchExpiry;

			var batchQty = $(this).find(":selected").data("batchqty");
			qtitems[item_id].row.batchQuantity = batchQty;

			var batchPurchaseCost = $(this)
				.find(":selected")
				.data("batchpurchasecost");
			qtitems[item_id].row.batchPurchaseCost = batchPurchaseCost;

			var batchSalePrice = $(this).find(":selected").data("batchsaleprice");
			qtitems[item_id].row.price = batchSalePrice;

			qtitems[item_id].row.batch_no = new_batchno;
			localStorage.setItem("qtitems", JSON.stringify(qtitems));
			loadItems();
		});
	...
	$(document)
		...
		.on("change", ".rbonus", function () {
			...
			var new_bonus = parseFloat($(this).val()),
				item_id = row.attr("data-item-id");
			qtitems[item_id].row.bonus = new_bonus;
			localStorage.setItem("qtitems", JSON.stringify(qtitems));
	...
	$(document)
		...
		.on("change", ".rquantity", function () {
			...
			var new_qty = parseFloat($(this).val()),
				item_id = row.attr("data-item-id");
			//qtitems[item_id].row.base_quantity = new_qty;
			if (qtitems[item_id].row.unit != qtitems[item_id].row.base_unit) {
				$.each(qtitems[item_id].units, function () {
					if (this.id == qtitems[item_id].row.unit) {
						//qtitems[item_id].row.base_quantity = unitToBaseQty(new_qty, this);
					}
				});
			}
			qtitems[item_id].row.qty = new_qty;
			localStorage.setItem("qtitems", JSON.stringify(qtitems));
	...
	$(document)
		...
		.on("change", ".rcost", function () {
			...
			var new_price = parseFloat($(this).val()),
				item_id = row.attr("data-item-id");
			qtitems[item_id].row.price = new_price;
			qtitems[item_id].row.actual_prod_price = new_price;
			qtitems[item_id].row.net_unit_sale = new_price;
			localStorage.setItem("qtitems", JSON.stringify(qtitems));
	...
	$(document)
		...
		.on("change", ".scost", function () {
			...
			var new_price = parseFloat($(this).val()),
				item_id = row.attr("data-item-id");
			qtitems[item_id].row.cost = new_price;
			localStorage.setItem("qtitems", JSON.stringify(qtitems));
			loadItems();
		});

	$(document).on("click", "#removeReadonly", function () {
		$("#qtcustomer").select2("readonly", false);
...
function loadItems() {
	...
	if (localStorage.getItem("qtitems")) {
		...
		warning_note = 0;
		$("#slTable tbody").empty();
		qtitems = JSON.parse(localStorage.getItem("qtitems"));
		sortedItems =
			site.settings.item_addition == 1
				? _.sortBy(qtitems, function (o) {
		...
		$.each(sortedItems, function () {
			...
			var total_before_dis_vat =
				parseFloat(item_sale_price) * parseFloat(item_qty); //(parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty);
