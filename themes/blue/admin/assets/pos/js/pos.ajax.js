$(document).ready(function () {
	...
	$(".po")
		.popover({ html: true, placement: "right", trigger: "click" })
		.popover();
...
$(document).ready(function () {
	...
	$(document).on("click", "#updateShipping", function () {
		var s = parseFloat(
			$("#shipping_input").val() ? $("#shipping_input").val() : "0"
		);
		if (is_numeric(s)) {
			$("#posshipping").val(s);
	...
	$(document).on("click", "#editComment", function () {
		...
		(positems[item_id].row.order = parseFloat($("#iorders").val())),
			(positems[item_id].row.comment = $("#icomment").val()
				? $("#icomment").val()
				: "");
		localStorage.setItem("positems", JSON.stringify(positems));
		$("#cmModal").modal("hide");
	...
	$(document).on("click", "#editItem", function () {
		...
		positems[item_id].row.option = $("#poption").val()
			? $("#poption").val()
			: "";
...
function loadItems() {
	if (localStorage.getItem("positems")) {
		...
		if (pos_settings.remote_printing != 1) {
			store_name =
				biller && biller.company != "-" ? biller.company : biller.name;
			...
			var pos_head2 =
				...
				"</p></span>";
			$("#order_span").prepend(
				styles + pos_head1 + " " + lang.order + " " + pos_head2
			);
			$("#bill_span").prepend(
				styles + pos_head1 + " " + lang.bill + " " + pos_head2
			);
			$("#order-table").empty();
			$("#bill-table").empty();
		}
		...
		$.each(sortedItems, function () {
			...
			const new_net_sale_after_discount = new Decimal(
			...
			new_grand_cost_goods_sold =
				new_grand_cost_goods_sold.plus(calc_cost_goods_sold);
			...
			if (site.settings.tax1 == 1) {
				if (pr_tax !== false && pr_tax != 0) {
					if (pr_tax.type == 1) {
						if (item_tax_method == "0") {
							pr_tax_val = formatDecimal(
								(unit_price * parseFloat(pr_tax.rate)) /
									(100 + parseFloat(pr_tax.rate)),
								4
							);
							...
							pr_tax_val = formatDecimal(
								(unit_price * parseFloat(pr_tax.rate)) / 100,
								4
							);
							pr_tax_rate = formatDecimal(pr_tax.rate) + "%";
						}
			...
			if (display_item_id && item.id == display_item_id) {
				cart["item"] = {
					...
					total: formatDecimal(
						(parseFloat(item_price) + parseFloat(pr_tax_val)) *
							parseFloat(item_qty)
					),
				};
			}

			var gtotal = parseFloat(
				total + invoice_tax - order_discount + parseFloat(shipping)
			);
			// new changes
			...
			tr_html +=
				...
				'">' +
				item_expiry +
				'</span><input name="item_expiry[]" type="hidden" value="' +
				item_expiry +
					...
					$.each(combo_items, function () {
						if (
							parseFloat(this.quantity) <
								parseFloat(this.qty) * base_quantity &&
							this.type == "standard"
						) {
							$("#row_" + row_no).addClass("danger");
						}
			...
			if (pos_settings.remote_printing != 1) {
				b_items +=
					product_name("#" + (an - 1) + " " + item_code + " - " + item_name) +
					"\n";
				...
				if (pos_settings.item_order == 1 && print_cate) {
					var bprTh = $("<tr></tr>");
					bprTh.html(
						'<td colspan="100%" class="no-border"><strong>' +
							item.row.category_name +
							"</strong></td>"
					);
					var oprTh = $("<tr></tr>");
				...
				bprTr +=
					...
					(item_discount != 0
						? "<del>" +
						  formatMoney(
								parseFloat(item_price) + parseFloat(pr_tax_val) + item_discount
						  ) +
						  "</del>"
						: "") +
					formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val)) +
		...
		if (posdiscount != null && posdiscount.indexOf("%") !== -1) {
			new_total_discount = new Decimal(
				new_total_discount.plus(order_discount)
		...
		new_total_discount = new Decimal(new_total_discount).toDecimalPlaces(
			5,
			Decimal.ROUND_DOWN
		);

		new_grant_total = new Decimal(
			new_grant_total.minus(new_total_discount)
		).toDecimalPlaces(5, Decimal.ROUND_DOWN);

		grand_total_net_sale = new Decimal(
			new_total_sale.minus(new_total_discount)
		).toDecimalPlaces(5, Decimal.ROUND_DOWN);
		...
		if (pos_settings.remote_printing != 1) {
			...
			if (site.settings.tax2 != 0 && invoice_tax != 0) {
				b_totals +=
					printLine(lang.order_tax + ": " + formatMoney(invoice_tax)) + "\n";
			}
			b_totals +=
				printLine(lang.grand_total + ": " + formatMoney(gtotal)) + "\n";
			if (pos_settings.rounding != 0) {
				...
				b_totals +=
					printLine(lang.rounding + ": " + formatMoney(rounding)) + "\n";
				b_totals +=
					printLine(lang.total_payable + ": " + formatMoney(round_total)) +
					"\n";
			}
			b_totals +=
				"\n" +
			...
			$("#bill_footer").append(
				'<p class="text-center"><br>' + lang.merchant_copy + "</p>"
			);
		}
		if (count > 1) {
			$("#poscustomer").select2("readonly", true);
		...
		if (
			display_item_id &&
			(display_item_id == "update" || (cart && cart.item))
		) {
...
function add_invoice_item(item) {
	...
	if (positems[item_id]) {
		...
		positems[item_id].row.qty = new_qty;
		if (
			!positems[item_id].row.serial_numbers.includes(item.row.serial_number)
		) {
...
function formatQuantity(x) {
	return x != null
		? '<div class="text-center">' +
				formatNumber(x, site.settings.qty_decimals) +
				"</div>"
		: "";
}
...
function formatMoney(x, symbol) {
	...
	if (site.settings.sac == 1) {
		return (
			symbol + "" + formatSA(parseFloat(x).toFixed(site.settings.decimals))
		);
	}
...
function is_float(mixed_var) {
	return (
		+mixed_var === mixed_var && (!isFinite(mixed_var) || !!(mixed_var % 1))
	);
}
...
function calculatePOSInventory(item) {
	const toTwoDecimals = (value) =>
		new Decimal(value).toDecimalPlaces(5, Decimal.ROUND_DOWN);
	...
	const net_sale_after_discount = toTwoDecimals(
		total_sale.minus(item_discount)
	);
	...
	if (tax_rate.equals(5)) {
		total_vat = toTwoDecimals(
			net_sale_for_vat.times(new Decimal(15).dividedBy(100))
		);
	}

	// discount percentage calculcation
	const new_discount1 = toTwoDecimals(
		toTwoDecimals(item_discount * 100).dividedBy(total_sale)
	);

	// Grant Total
	const grant_total = toTwoDecimals(net_sale.plus(total_vat));

	const grant_total_after_discount = toTwoDecimals(
		net_sale_after_discount.plus(total_vat)
	);
