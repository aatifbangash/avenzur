...
<script>
    $(document).ready(function () {
        oTable = $('#TOData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('stock_request/getincomingStockRequests') ?>',
            ...
            'fnRowCallback': function (nRow, aData, iDisplayIndex) {
                ...
                nRow.id = aData[0];
                nRow.className = "stock_request_link";
                return nRow;
            },
    ...
    $(document).on('click', '.stock_status_update', function (e) {
        e.preventDefault;
        var row = $(this).closest('td').closest('tr');
        var id = row.attr('id');
        
            $('#myModal').modal({ remote: site.base_url + 'stock_request/incoming_update_status/' + id });
            $('#myModal').modal('show');
        
    });
    
    $('body').on('click', '.stock_request_link td:not(:first-child, :nth-last-child(3), :nth-last-child(2), :last-child)', function () {
        $('#myModal').modal({
            remote: site.base_url + 'stock_request/view/' + $(this).parent('.stock_request_link').attr('id'),
        });
        $('#myModal').modal('show');
    });
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-star-o"></i><?= lang('Stock Incoming Requests'); ?></h2>
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <!--<ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        ...
                        <li class="divider"></li>
                        <li>
                            <a href="#" class="bpo" title="<?= $this->lang->line('Delete Requests') ?>"
                             data-content="<p><?= lang('r_u_sure') ?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button>"
                             data-html="true" data-placement="left">
                             <i class="fa fa-trash-o"></i> <?= lang('Delete Requests') ?>
                         </a>
                     </li>
                 </ul>-->
             </li>
