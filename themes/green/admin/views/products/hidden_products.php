...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#PRData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('products/getHiddenProducts' . ($warehouse_id ? '/' . $warehouse_id : '') . ($supplier ? '?supplier=' . $supplier->id : '')) ?>',
...
<?php if ($Owner || ($GP && (!empty($GP['products-add']) || !empty($GP['products-update_price']) || !empty($GP['products-sync_quantity']) || !empty($GP['products-set_avg_cost']) || !empty($GP['products-export_excel']) || !empty($GP['products-delete']))) {
                echo admin_form_open('products/product_actions' . ($warehouse_id ? '/' . $warehouse_id : ''), 'id="action-form"');
} ?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-barcode"></i><?= lang('Hidden Products') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')' . ($supplier ? ' (' . lang('supplier') . ': ' . ($supplier->company && $supplier->company != '-' ? $supplier->company : $supplier->name) . ')' : ''); ?>
        </h2>
