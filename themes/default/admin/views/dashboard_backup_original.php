...
.dashboard-container { padding: 1.5rem 0; max-width: 100%; }
.dashboard-header { margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid var(--border-color); }
.dashboard-header h1 { font-size: 2rem; font-weight: 700; color: var(--dark); margin-bottom: 0.5rem; }
.dashboard-header p { font-size: 1rem; color: #6c757d; margin: 0; }
...
.stat-content { flex: 1; }
.stat-label { font-size: 0.875rem; color: #6c757d; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; margin-bottom: 0.5rem; }
.stat-value { font-size: 2.5rem; font-weight: 700; color: var(--dark); line-height: 1; margin-bottom: 0.5rem; }
.stat-change { font-size: 0.875rem; display: flex; align-items: center; gap: 0.5rem; }
...
@media (max-width: 768px) {
    ...
    .stat-icon { width: 60px; height: 60px; font-size: 2rem; margin-left: 0.5rem; }
...
function row_status($x)
{
    if ($x == null) {
        ...
        return '<span class="badge" style="background-color: #ffc107; color: #000;">'.lang($x).'</span>';
    } elseif ($x == 'completed' || $x == 'paid' || $x == 'sent' || $x == 'received') {
        return '<span class="badge" style="background-color: #198754; color: #fff;">'.lang($x).'</span>';
    } elseif ($x == 'partial' || $x == 'transferring') {
        return '<span class="badge" style="background-color: #0dcaf0; color: #fff;">'.lang($x).'</span>';
    } elseif ($x == 'due') {
        return '<span class="badge" style="background-color: #dc3545; color: #fff;">'.lang($x).'</span>';
    }
    return '<span class="badge" style="background-color: #6c757d; color: #fff;">'.lang($x).'</span>';
}
?>
...
<div class="dashboard-container">
    <div class="dashboard-header">
        ...
        <p><?= lang('welcome_back') ?>, <?= $this->session->userdata('username') ?>!</p>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            <div class="stat-content">
                ...
                <div class="stat-value"><?php 
                    ...
                    echo $this->sma->convertNumber($total_sales);
                ?></div>
        ...
        <div class="stat-card">
            <div class="stat-content">
                <div class="stat-label"><?= lang('purchases') ?></div>
                <div class="stat-value"><?php 
                    $total_purchases = 0;
                    if($purchases) foreach($purchases as $purchase) $total_purchases += $purchase->total;
                    echo $this->sma->convertNumber($total_purchases);
                ?></div>
                <div class="stat-change positive">
                    <span>↑ Recent purchases</span>
                </div>
            </div>
            <div class="stat-icon success"><i class="fa fa-box"></i></div>
        </div>

        <!-- Quotes Card -->
        <div class="stat-card">
            <div class="stat-content">
                <div class="stat-label"><?= lang('quotes') ?></div>
                <div class="stat-value"><?php echo isset($quotes) && is_array($quotes) ? count($quotes) : 0; ?></div>
                <div class="stat-change positive">
                    <span>↑ Active quotes</span>
                </div>
            </div>
            <div class="stat-icon warning"><i class="fa fa-file-text"></i></div>
        </div>

        <!-- Stock Value Card -->
        <div class="stat-card">
            <div class="stat-content">
                <div class="stat-label"><?= lang('stock_value') ?></div>
                <div class="stat-value"><?php echo isset($stock) ? $this->sma->convertNumber($stock->total) : '0'; ?></div>
                <div class="stat-change positive">
                    <span>↑ Current inventory</span>
                </div>
            </div>
            <div class="stat-icon info"><i class="fa fa-cubes"></i></div>
        </div>
    </div>
